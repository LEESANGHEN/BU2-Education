...
/* ── 상태 ── */
var S={filterSite:'all',filterSites:[],showHidden:false,groups:[],sites:[],projects:[],schedules:[],events:[],workTasks:[],equipItems:[],equipUnits:[],equipSiteOrder:[],equipProjects:[],visionTemplate:{categories:[]},visionEquips:[],masterProjects:[]};
/* ══════════════════════════════════════════
   ...
   - type = 엔티티 배열명: schedules, events, workTasks, sites, groups,
     projects, equipItems, equipUnits, equipProjects, visionEquips, masterProjects
══════════════════════════════════════════ */
var _TOMB_LS_KEY='bu2_tomb_v1';
var _tombMap=(function(){
  var m={};
  try{m=JSON.parse(localStorage.getItem(_TOMB_LS_KEY)||'{}')||{};}catch(e){m={};}
  // 구 키(bu2_del_sc / bu2_del_vi)에서 1회 마이그레이션
  try{
    var oldSc=JSON.parse(localStorage.getItem('bu2_del_sc')||'{}');
    var oldVi=JSON.parse(localStorage.getItem('bu2_del_vi')||'{}');
    if(Object.keys(oldSc).length){m.schedules=m.schedules||{};Object.keys(oldSc).forEach(function(id){if(!m.schedules[id])m.schedules[id]=oldSc[id];});localStorage.removeItem('bu2_del_sc');}
    if(Object.keys(oldVi).length){m.visionEquips=m.visionEquips||{};Object.keys(oldVi).forEach(function(id){if(!m.visionEquips[id])m.visionEquips[id]=oldVi[id];});localStorage.removeItem('bu2_del_vi');}
  }catch(e){}
  // TTL 30일 정리
...
var _TOMB_CLEAR_LS_KEY='bu2_tomb_clear';
...
function loadData(){
  ...
  try{S.showHidden=localStorage.getItem('bu2_showHidden')==='1';}catch(e){}
...
function _saveLocalBackup(){
  try{
    var snap={ts:Date.now(),
      ...
      sites:S.sites, groups:S.groups, projects:S.projects, masterProjects:S.masterProjects};
...
function downloadDataBackup(){
  try{
    var data={ts:new Date().toISOString(),
      ...
      visionTemplate:S.visionTemplate,visionEquips:S.visionEquips,masterProjects:S.masterProjects};
    ...
    var d=new Date();
    a.href=url;
    a.download='BU2_backup_'+d.getFullYear()+String(d.getMonth()+1).padStart(2,'0')+String(d.getDate()).padStart(2,'0')+'_'+String(d.getHours()).padStart(2,'0')+String(d.getMinutes()).padStart(2,'0')+'.json';
...
══════════════════════════════════════════ */
var DATA_ISSUES_KEY='bu2_data_issues';
...
var DEVICE_TAG_KEY='bu2_device_tag';
...
function loadFromSheets(callback){
  ...
  fetch(url+'?action=load')
    ...
    .then(function(data){
      ...
      // ── equipSiteOrder / equipProjects / masterProjects ──
      if(data.equipSiteOrder&&data.equipSiteOrder.length)S.equipSiteOrder=data.equipSiteOrder;
